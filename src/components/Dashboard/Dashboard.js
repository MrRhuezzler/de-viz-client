...
import OptionsLayoutProvider, {
  layoutContext,
  generateLayout,
  optionsContext,
} from "../../hooks/optionsLayoutContext";
...
const DashboardComponent = ({ layoutItem }) => {
  ...
  return (
    <>
      <div
        className={`flex flex-row items-center h-[50px] justify-between py-2 px-4 bg-${
          options.find((e) => e.i === layoutItem.i).accentColor
        }`}
        style={{ cursor: "move" }}
      >
        <h1
        ...
        >
          {options.find((e) => e.i === layoutItem.i).title}
        </h1>
        <div className="space-x-2">
          <button
            className="rounded-full p-2 bg-white font-bold"
            onClick={(e) => {
              setToggleChangeColor(!toggleChangeColor);
            }}
          >
      ...
      <div
        className={`${
          toggleChangeColor ? "" : "hidden"
        } flex flex-row justify-center p-2 space-x-2`}
      >
        {Object.keys(accentColors).map((color, index) => (
          <button
            ...
            onClick={(e) => {
              ...
              setOptions(
                options.filter((e) => e.i !== layoutItem.i).concat(currOptions)
              );
            }}
            className={`w-[20px] aspect-square rounded-full bg-${color}`}
          ></button>
        ))}
      </div>
      {/* } */}
...
const DashboardParent = (props) => {
  ...
  const getGridLayoutWithOptions = (onlyGridlayout, options) => {
    ...
    for (const key of Object.keys(onlyGridlayout)) {
      gridLayoutWithOptions[key] = onlyGridlayout[key].map((l) => ({
        gridLayout: onlyGridlayout,
        options,
      }));
    }
  ...
  return (
    <div className="bg-white-smoke h-full">
      <ResponsiveReactGridLayout
        ...
        {...props}
        layouts={layout}
        onDrag={(layouts, oldItem, newItem, placeholder, event, element) => {
          setCurrBackgroundColor(
            options.find((e) => e.i === newItem.i).accentColor
          );
        }}
        onResize={(layouts, oldItem, newItem, placeholder, event, element) => {
          setCurrBackgroundColor(
            options.find((e) => e.i === newItem.i).accentColor
          );
        }}
